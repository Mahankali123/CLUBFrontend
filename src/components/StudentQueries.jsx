...
const StudentQueries = () => {
  ...
  useEffect(() => {
    const fetchQueries = async () => {
      try {
        const response = await axios.get('https://club-backend-tau.vercel.app/api/GetQuery'); // Using CORS proxy
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://club-backend-tau.vercel.app/api/deleteQuery/${id}`);
